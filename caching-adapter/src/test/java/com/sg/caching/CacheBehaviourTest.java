 ...
 * testable this way is the reason {@code PartyReferentialService} is narrower than the referential's own
 * interface.
 */
class CacheBehaviourTest {
    ...
    private static PartyReferentialService counting(AtomicInteger calls,
    ...
    void upstreamFailureIsNotCachedAndIsReportedAsRetryable() {
        var calls = new AtomicInteger();
        PartyReferentialService failing = new PartyReferentialService() {
            @Override
            public List<PartyRegistrationDetails> search(PartySearchRequest r) {
                calls.incrementAndGet();
                throw new ReferentialUnavailableException("party-registration", "endpoint down", true, null);
            }
        };
