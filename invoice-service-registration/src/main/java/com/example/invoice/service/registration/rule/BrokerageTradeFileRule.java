...
public final class BrokerageTradeFileRule implements ValidationRule {
  ...
  public List<MappingError> check(ValidationContext ctx) {
    ...
    boolean hasTradeFile = false;
    for (ExtractedAttachment a : ctx.multipartAttachments()) {
      if (isTradeFile(a)) { hasTradeFile = true; break; }
    }
    if (!hasTradeFile) {
      for (ExtractedAttachment a : ctx.jsonAttachments()) {
        if (isTradeFile(a)) { hasTradeFile = true; break; }
      }
    }
    if (hasTradeFile) return List.of();
